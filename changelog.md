...
* Added diagnostics around generated-only trashcan removal and subsequent mining-plan placement to identify cleared accessway origins that remain live or are recreated before a failed access search.
...
* Fixed: ocean-avoidance cutting rays no longer reject an ocean sample when the projected slope has already met terrain there and requires no work. Positive-work cutting spans and their configured post-termination safety margin remain ocean-blocked.
