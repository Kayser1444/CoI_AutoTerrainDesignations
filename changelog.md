...
* Changed: Accessways ignore already buried props and classify every remaining prop against the exact final height at its position. The normal accessway operation owns props it will remove itself; otherwise the manager buries before mining, excavates before dumping, or chooses the lowest-cost workable direction before leveling, then restores the intended accessway designation. Known limitation: preparatory landscaping can invalidate the planned path, and that path is not yet replanned or revalidated after cleanup.
* Added: The accessway-only **Quick remove debris** policy offers Always, Restrictive, and Never under Accessways. Restrictive uses Quick remove only when the default accessway operation cannot remove the prop; Always queues Quick remove for every non-buried accessway prop. Insufficient Unity leaves a persisted Quick request pending for a once-per-second retry instead of failing or silently falling back to terrain work.
