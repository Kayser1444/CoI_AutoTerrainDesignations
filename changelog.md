...
* Added: AUTO scans can use an unstarted mine control tower ghost as a natural accessway marker. When no terrain work already exists, ATD combines exact-terrain vehicle-pathable approaches to tower ghosts whose entrances are inside the active tower area into one multi-source search and immediately commits the route that reaches a goal. A recognized ghost claims the AUTO request even if routing or placement fails, so the request cannot fall through into an unrelated useful-product scan. Ghosts remain vanilla-owned and are never created, modified, removed, or persisted by ATD.
