...
## Verified vanilla terrain-change removal rules

These rules are from the decompiled vanilla managers that react to terrain-height changes. They are distinct from vanilla quick-remove prop commands, which directly delete props/stumps and charge Unity; ATD accessway cleanup and path-costing must not model quick-remove as a free or implicit pathfinding action.

For non-tree terrain props (`TerrainPropsManager`):

* Height-change pruning only evaluates a prop when the changed terrain tile maps to that prop. The removal test samples terrain height at the prop's exact floating-point `TerrainPropData.Position`, not at every tile in the prop's occupied footprint.
* Mining/lowering removes the prop when `currentHeightAtPropPosition - PlacedAtHeight` is strictly below `PlacementHeightOffset`. For normal generated props this offset is zero, so any real lowering below the placed height removes the prop. Equal height does not.
* Dumping/raising removes the prop when `currentHeightAtPropPosition - PlacedAtHeight` is strictly greater than `DespawnBuriedThreshold.ScaledBy(Scale)`. Equal height at the buried threshold does not remove it.
* Base terrain props registered in `Mafi.Base` use `DespawnBuriedThreshold = 0.5` tile before instance scaling. This includes the stone props, `BushSmall`, and `BushMedium`; `BushSmall` is explicitly non-vehicle-blocking.
* `ProductWhenHarvested` is only for vehicle harvest/removal paths. Terrain-change destruction does not imply harvested products.

For trees and stumps (`TreesManager`):

* Height-change pruning evaluates the tree/stump whose `TreeId` is the changed tile. It samples the tile height at that terrain index and compares it to the tree/stump `PlantedAtHeight`.
* A live tree collapses when terrain is raised at least `+1.0` tile or lowered at least `-0.5` tile. Equality counts for both thresholds.
* A stump collapses when terrain is raised or lowered at least `0.25` tile from its planted height. Equality counts.
* `TryRemoveTree` normally creates a stump. In the height-change path, the subsequent stump check runs in the same handler, and the tree-collapse thresholds already exceed the stump-collapse thresholds; therefore a terrain change large enough to collapse a live tree should also remove the newly-created stump during the same terrain-height event.

Implication for ATD: the current one-level stub is only an approximation. It is too coarse for tree mining (`0.5` tile is enough), non-tree dumping (usually strictly greater than `0.5 * scale`), and non-tree mining (any strict cut at the prop position is enough). It is also spatially conservative/imprecise because ATD reasons in 4x4 origins and occupied sample tiles, while vanilla's terrain-change tests are triggered from the prop/tree tile and may sample the exact prop position.

## Desired behavior

Treat removable debris as a low-cost property of a ground node, not as a reason to manufacture terrain work. The one exception is genuine terrain work that will remove the prop anyway: sufficiently large terrain changes can clear props without a separate cleanup action, using the verified vanilla thresholds above. This matters most for generated `V`: the route may ignore a blocking prop only when the planned terrain edit actually crosses that prop class's terrain-change removal threshold.

ATD deliberately applies a slightly simpler operational policy on top of the exact vanilla thresholds:

* **Non-tree props never block mining or leveling.** In practice, any mining designation removes them; this is the mechanism ATD already uses for standalone debris cleanup. A mining or leveling `V` candidate that overlaps a removable non-tree prop is legal and pays the debris-removal landscaping cost once for that origin.
* **Non-tree props block dump-only work unless the dump itself removes them.** If the planned dumping rises strictly past the scaled buried threshold, the prop is destroyed by the dump and the candidate is legal. If the candidate only dumps onto the prop without reaching that threshold, reject it as a hard blocker. The player could manually mine first and dump later, but the accessway pathfinder cannot currently express that two-step sequence inside one `V` candidate.
* **Trees never block the path search.** Every traversed tree is recorded for harvest because ATD prefers recovering the wood when possible. Charge tree harvest/cleanup cost only when the tree lies within the non-destructive range of the planned work. If the generated terrain edit will destroy the tree by crossing the verified tree threshold, keep the harvest metadata for best-effort materialization but do not charge the harvest cost as a required cleanup cost.
...
2. Traversing that node pays normal G traversal length plus a small one-time debris cleanup landscaping cost.
   Consecutive cleanup-ground tiles may continue through the same cleanup object without manufacturing an intervening `V` cell. This includes one prop occupying several tiles and one tree whose blocked samples cross a 4x4 origin boundary. Cleanup ground must not chain into an unrelated cleanup object merely because the tiles are adjacent.
   Tree-only cleanup ground may continue into adjacent tree-only cleanup ground belonging to another tree, because trees never force generated terrain work; each newly encountered tree still contributes its own harvest metadata and cleanup cost. A tree sample overlapping already ordinary vanilla-pathable `G` is not required cleanup and must not trigger harvesting merely because the selected route passes nearby.
3. A generated `V` node may overlap removable non-tree props without a separate cleanup action whenever its operation includes mining or leveling. Dump-only `V` may overlap a removable non-tree prop only when the dump reaches the verified buried-removal threshold; otherwise the prop remains a hard blocker.
4. Tree props are routable in both `G` and `V`. They always add harvest metadata. They add harvest/cleanup cost only when the planned `V` work is non-destructive for that tree, or when the route remains in `G`.
5. The cleanup cost is controlled by the public global `accessPropCleanupLandscapingCost` mod parameter. One unit of **landscaping cost** is the equivalent of dumping or digging one unit of rock. The default is `8`, calibrated from observed excavator cleanup effort. The incremental cost is `landscapingCostDistanceScale * accessPropCleanupLandscapingCost` once per affected debris designation origin, not once per tile step through the same origin.
6. Successful path materialization emits debris-removal mining designations for debris origins used by the accepted path, using the same target profile as the standalone cleanup flow: current corner surface height plus one.
7. Clearing debris makes the affected ground usable; it does **not** create a `V` provider, alter the ground height, consume a generated accessway origin, or participate in accessway geometry checks.
8. Generated debris cleanup designations are mining protos **in the air**. They remove props; they are not terrain-shaping V mining designations and must not be treated as fixed profiles, fight-invariant neighbors, or durability sources.
...
* `removablePropMiningRemoves`: an operation-level rule that treats mining and leveling as sufficient to remove removable non-tree props. The exact vanilla rule is stricter (`plannedHeight < PlacedAtHeight + PlacementHeightOffset`), but ATD's practical cleanup mechanism is an in-air mining designation and experience shows any mining designation clears these props.
* `removablePropDumpThreshold`: true when the planned height at the prop/sample position is strictly above `PlacedAtHeight + DespawnBuriedThreshold.ScaledBy(Scale)`. Base debris props use a half-tile threshold before instance scaling. Dump-only candidates that do not satisfy this are hard-blocked by the prop.
* `treeMiningRemoveThreshold` / `treeDumpThreshold`: live trees are destroyed by lowering at least `0.5` tile or raising at least `1.0` tile from planted height; equality counts. Trees are never hard blockers, but these thresholds decide whether tree harvest cost is required or only best-effort harvest metadata is recorded.
* `stumpMiningRemoveThreshold` / `stumpDumpThreshold`: stumps are removed by lowering or raising at least `0.25` tile from planted height; equality counts. A terrain change that removes a live tree should also remove the stump created during that same height-change event.
...
* Generated `V` candidates evaluate prop-occupied sample tiles separately from `G`: a removable non-tree prop inside a mining or leveling candidate is legal and pays removal cost; inside a dump-only candidate it is legal only if dumping rises strictly past the scaled buried threshold. A dump-only candidate below that threshold is rejected as prop-blocked. Tree props are always legal; they carry harvest metadata, and they pay harvest cost only if the candidate remains inside the non-destructive range.
...
* **Dump-only generated V over non-tree props below the buried threshold.** A dump-only V candidate that does not remove a non-tree prop must be rejected because the prop remains path-blocking after the dump.
* **Zero-work generated V over non-tree props.** A generated V candidate that does not perform prop-removing terrain work, and does not carry cleanup metadata for the occupied prop origins, must not ignore non-tree blocking props merely because it is in V mode.
...
Before placement, rematerialization must revalidate that each cleanup origin still has the blocking prop classes recorded by the selected path, is still fully inside the area, and still has no terrain designation. For generated `V` paths that rely on terrain work to remove props, rematerialization must also re-check that the final target profile still performs the required prop-removing work: mining/leveling for removable non-tree props, threshold-clearing dumping for removable non-tree props, and threshold-clearing mining/dumping for trees when tree harvest cost was not charged. If a cleanup origin became unnecessary because all relevant props disappeared, drop that cleanup item and continue. If only some classes disappeared from a mixed origin, keep the remaining cleanup actions and update diagnostics. If any cleanup origin became unsafe or conflicted with a terrain designation, reject the candidate and let the caller search again against a fresh snapshot or fall back.

Placement should use the mining proto and target `surfaceHeight + 1` at each of the four designation corners for debris cleanup, matching the existing cleanup flow. Tree cleanup should prefer the forestry harvest mechanism verified from AFD: resolve the `TreeId`s in the accepted cleanup footprint, call the tree manager's harvest-selection operation for each unselected tree, and activate/refresh the harvest overlay if needed. Mark trees for harvest even when the planned terrain work is expected to destroy them, because wood should be recovered if workers reach the harvest first; in that destructive case the harvest is best-effort metadata and should not be charged as a required cleanup cost. If the game cannot safely express tree harvesting together with debris cleanup for the same origin, the mixed origin should fall back to the debris cleanup action that removes all blocking non-tree props while still marking trees best-effort, or be rejected and re-searched if the remaining non-tree props cannot be cleared. Treat these placements/actions as cleanup work items rather than terrain profiles: they should be registered for ownership and rollback, but not reintroduced into the access snapshot as access providers, fixed V profiles, fight-invariant blockers, or durability-envelope sources. Successful debris cleanup placements should be registered through the same generated-designation ownership path used by ATD-generated mining/accessway designations so rollback, tower ownership, and save-removability behavior stay consistent.
...
* A zero-work generated `V` profile over blocking non-tree props is rejected or requires cleanup metadata; it must not ignore those props merely because it is `V`.
* A generated mining or leveling `V` profile may overlap removable non-tree props, pays debris-removal landscaping cost for the affected origin, and does not hard-block on the exact non-tree cut threshold.
* A generated dump-only `V` profile may overlap removable non-tree props only when the dump crosses the verified buried-removal threshold; below that threshold the prop remains a hard blocker.
* A generated `V` profile may overlap trees regardless of destruction threshold, records harvest metadata, and charges harvest cost only when the planned terrain work remains within the non-destructive tree range.
