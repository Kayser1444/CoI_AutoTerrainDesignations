...
* **V2 -> G handoff.** A V2 path may leave generated V-space only through a width-2 workable frontage on one side of the terminal 2x2 brush. The exposed side has two adjacent origin edges, one per lane. Each lane proves the corner-crest rule below, and the complete eight-file handoff corridor must contain a cardinal post-work path from rank two to captured G. Rank one is already proved by the V-facing seam. Files 1-2 and 7-8 are clearance-only; the flood starts from any pathable rank-two center in files 3-6 and may use only those four middle files through every origin in a multi-origin span.
* **G -> V2 handoff.** Entering generated V2 from G is symmetric. Search views the candidate bands back toward G and applies the same lane-span corner-crest rule. A failed one-origin seam may extend straight through as many as `1 + ceil(vehicle clearance / 4)` additional eligible origins (three for the current five-tile Mega footprint), then test the complete span. Search and replay retain every generated origin in that span.
* **Corner-crest operation selection.** For each lane, compare the two extreme corners of the span's V-facing edge with natural terrain and compare the two extreme corners of its G-facing edge. If both G-facing corners are within height epsilon of natural terrain, select leveling. Otherwise select mining when both V-facing corners are at or below terrain, at least one is below, and both G-facing corners are at or above terrain. Select dumping for the exact inverse. Any same-side or mixed-face result is not a handoff. For a multi-origin span, the V-facing corners come from the innermost origin and the G-facing corners from the outermost origin: all four extreme corners must prove the crest.
* **No vanilla bitmap gate.** V2 does not reconstruct the prospective proto's vanilla 25-bit fulfilled bitmap. The corner crest selects the terminal operation and proves rank one; the explicit post-work files-3-through-6 flood proves that the selected operation actually leaves a usable route from rank two to G.
* **Post-work center classification.** Inside the four middle files, leveling centers are always pathable. A mining center is pathable when the terrain-only vanilla Mega mask passes with props ignored, or when that mining cell works the center (`target < ground`). A dumping center is pathable when the ordinary vanilla Mega result passes with trees ignored and no uncleared non-tree prop, or when that dumping cell works the center (`target > ground`). A non-tree prop may also cease blocking dumping only when the prop's actual occupied tiles extend into a neighboring 4x4 origin that remains free to host a persistent cleanup designation. The neighboring origin must not be active/fixed work, generated V, another handoff designation, a building footprint, or a reserved origin. Cleanup on the dumping origin would be replaced and does not qualify. With no such free occupied neighbor, the prop is a hard blocker for every handoff center it blocks. This classification is applied to every rank of every origin in the terminal span before the cardinal path-out flood is accepted.
...
* **Exact-ground profiles.** V2 has no separate no-work/`None` handoff case. An outbound edge already on natural ground is the leveling case; if the preceding origin already exposed the same usable seam, normal cheapest-label dominance makes the later match redundant.
...
```text
V2 handoff = two adjacent corner-crest handoffs on the same brush side
             + compatible terminal profiles
             + rank-2 start in files 3..6
             + operation-specific post-work path through the full span
             + captured clearance-2 G entry beyond the G-facing rank
```
