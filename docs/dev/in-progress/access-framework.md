...
V2 has separate band/profile, transition, parent-plus-delta history, Mega ground graph, frontage, seam, replay, and materialization types under `Access/V2`. A validated V-to-G seam enters explicit cardinal G states carrying the same history; tower-ground requests use A* with unit G travel in the primary cost, a canonical-center V lower bound, and the ground-distance lower bound while in G. Fixed-frontage requests retain Dijkstra. `[ATD V2 Search]` reports the actual algorithm, cost breakdown, V route, G suffix, and handoff. Accepted routes are replayed before transactional designation placement and post-placement Mega-seam validation.
...
* Extend the now-available per-world **Avoid ocean**, **Avoid buildings**, and **Harvest disrupted trees** Pathfinder parameters to ordinary mining-designation generation. Their new-game defaults come from `ATDsettings.json`, while current values persist in the removable state blob. Accessway candidate rays already enforce the avoidance options with the material-aware tracer. Mining generation should reject projected cutting into ocean when Avoid ocean is enabled, allow dumping/fill into ocean, and evaluate projected cut/fill near buildings rather than using a coarse hourglass. Keep direct building footprint/vehicle-clearance checks separate. Harvest disrupted trees should mark the finalized mining body and projected disturbance zones with tower ownership; when disabled, mark only execution-required trees. When an avoidance option is disabled, allow the plan but warn if projected work reaches the corresponding hazard; building warnings cover existing, planned, or ghost buildings, including the mine control tower itself.
