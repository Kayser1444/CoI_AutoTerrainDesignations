...
namespace AutoTerrainDesignations
{
    public static partial class AutoDepthDesignation
    {
            ...
            ReducedAccessDomainPlan? reducedDomain = null)
        {
            ...
            // available and must not observe this production lease.
            if (!AccessSearchFixtureGate.EnsureInitialized(
                    out string fixtureFailure))
            {
                output.FailureReason = "AccessFixtureGate: " + fixtureFailure;
                yield break;
            }
            IDisposable? captureLease = AccessCaptureBackpressure.TryAcquire();
            if (captureLease == null)
            {
                output.FailureReason = "SnapshotBusy";
                yield break;
            }

            try
            {
                IEnumerator capture = BuildExperimentalAccessSnapshotCore(
                    tower,
                    tileDepths,
                    cornerHeights,
                    terrMgr,
        ...
        private static IEnumerator BuildExperimentalAccessSnapshotCore(
            ...
            ReducedAccessDomainPlan? reducedDomain)
        {
            ...
            AccessSearchPolicySnapshot policy =
                AccessSearchPolicySnapshot.Capture();
            var captureSettingsRevision =
                new AccessRequestSettingsRevision(
                    GetOrCreateTowerSettings(tower).RampWidth,
                    GetTowerVehicleClearance(tower),
                    policy.SemanticFingerprint);
            var captureRevision = new AccessCaptureRevision(
                CurrentWorldGeneration,
                CurrentTerrainDesignationRevision,
                policy.SemanticFingerprint);
            long memoryCeilingBytes = policy.SnapshotMemoryCeilingMiB
                * 1024L * 1024L;
            var captureDiagnostics = new AccessCaptureDiagnostics(
                captureRevision,
                memoryCeilingBytes);
            var captureBudget = new AccessCaptureMemoryBudget(
                memoryCeilingBytes);
            output.CaptureDiagnostics = captureDiagnostics;
            ...
            if (!AccessSearchFixtureGate.EnsureInitialized(out failureReason))
            {
                ...
                captureDiagnostics.SetTerminalReason(output.FailureReason);
                yield break;
            }
            ...
            if (!IsWorldGenerationActive(captureRevision.WorldGeneration))
            {
                output.FailureReason = "CaptureInvalidated";
                captureDiagnostics.SetTerminalReason(output.FailureReason);
                yield break;
            }
            ...
            AccessCapturedBuildingFacts buildingFacts =
                reducedDomain == null
                    ...
                    : AccessCapturedBuildingFacts.Capture(
                        s_buildingOccupiedTiles.Where(
                            reducedDomain.CaptureTiles.Contains),
                        s_buildingFixedHeights2ByTile.Where(pair =>
                                reducedDomain.CaptureTiles.Contains(pair.Key))
                            .ToDictionary(pair => pair.Key, pair => pair.Value),
                        s_layoutEntityOccupanciesByTile.Where(pair =>
                                reducedDomain.CaptureTiles.Contains(pair.Key))
                            .ToDictionary(pair => pair.Key, pair => pair.Value));
            ...
            {
                boundsMin = reducedDomain.SearchOrigins.BoundsMin;
                boundsMax = reducedDomain.SearchOrigins.BoundsMax;
                groundCaptureMin = reducedDomain.CaptureTiles.BoundsMin;
                groundCaptureMax = reducedDomain.CaptureTiles.BoundsMax;
            }
            ...
            long captureHeight = Math.Max(
                0L,
                (long)groundCaptureMax.Y - groundCaptureMin.Y + 1L);
            long captureTileCount = reducedDomain != null
                ? reducedDomain.CaptureTiles.Count
                : captureWidth > 0
                    && captureHeight > 0
                    && captureWidth <= long.MaxValue / captureHeight
                        ? captureWidth * captureHeight
            ...
            long originHeight = Math.Max(
                0L,
                ((long)boundsMax.Y - boundsMin.Y) / 4L + 2L);
            long originCount = reducedDomain != null
                ? reducedDomain.SearchOrigins.Count
                : originWidth > 0
                    && originHeight > 0
                    && originWidth <= long.MaxValue / originHeight
                        ? originWidth * originHeight
                        : long.MaxValue;
            int coveredTileDepthCount = reducedDomain == null
                ? tileDepths.Count
                : (int)Math.Min(
                    Math.Min(originCount, int.MaxValue), tileDepths.Count);
            long initialEstimate =
                EstimateExperimentalSnapshotMemory(
                    captureTileCount,
                    originCount,
                    coveredTileDepthCount,
            ...
            {
                output.FailureReason = "SnapshotTooLarge";
                captureDiagnostics.SetTerminalReason(output.FailureReason);
                LogExperimentalAccessDebug(
                    "[ATD Access Capture] terminal=SnapshotTooLarge phase=initial "
                    + captureDiagnostics.Format());
                yield break;
            }

            bool CaptureStillValid()
            {
                if (!IsWorldGenerationActive(captureRevision.WorldGeneration))
                {
                    output.FailureReason = "CaptureInvalidated";
                ...
                AccessCaptureRevision currentRevision = new AccessCaptureRevision(
                    CurrentWorldGeneration,
                    CurrentTerrainDesignationRevision,
                    AccessSearchPolicySnapshot.Capture().SemanticFingerprint);
                var currentSettingsRevision =
                    new AccessRequestSettingsRevision(
                        GetOrCreateTowerSettings(tower).RampWidth,
                        GetTowerVehicleClearance(tower),
                        currentRevision.PolicyFingerprint);
                if (AccessCaptureRevisionPolicy.Classify(
                        captureRevision, currentRevision)
                    != AccessCaptureInvalidationKind.HardInvalidation)
                {
                    if (currentSettingsRevision != captureSettingsRevision)
                    {
                        output.FailureReason = "CaptureInvalidated";
                        captureDiagnostics.SetTerminalReason(
                            "CaptureInvalidated:AccessSettingsChanged");
                        return false;
                    }
                    return true;
                }
            ...
            int coveredDesignationCapacity = reducedDomain == null
                ? designationCapacity
                : (int)Math.Min(
                    designationCapacity,
                    Math.Min(captureTileCount, int.MaxValue));
            long fixedProfileCapacityLong = Math.Min(
                originCount,
                (long)designationCapacity + coveredTileDepthCount);
            ...
            var rayDesignations =
                new Dictionary<Tile2i, TerrainDesignation>(
                    coveredDesignationCapacity);
            ...
            foreach (TerrainDesignation designation in capturedDesignations)
            {
                ...
                if (origin.X >= fixedDesignationMin.X
                    ...
                    && (reducedDomain == null
                        || reducedDomain.SearchOrigins.Contains(origin)))
                {
                ...
                if (origin.X >= rayDesignationMin.X
                    ...
                    && (reducedDomain == null
                        || CoverageIntersectsDesignation(
                            reducedDomain.CaptureTiles, origin)))
                {
            ...
            captureStepStart = AtdDiagnostics.Timestamp();
            IEnumerable<Tile2i> captureTiles = reducedDomain != null
                ? reducedDomain.CaptureTiles.EnumerateTiles()
                : EnumerateAccessRectangle(groundCaptureMin, groundCaptureMax);
            foreach (Tile2i tile in captureTiles)
            {
            ...
            captureStepStart = AtdDiagnostics.Timestamp();
            IEnumerable<Tile2i> searchOrigins = reducedDomain != null
                ? reducedDomain.SearchOrigins.EnumerateTiles()
                : EnumerateAccessOrigins(
                    firstOriginX, firstOriginY, boundsMax, 0);
            ...
            captureStepStart = AtdDiagnostics.Timestamp();
            IEnumerable<Tile2i> terrainColumnOrigins = reducedDomain != null
                ? EnumerateRequiredTerrainColumns(reducedDomain.SearchOrigins)
                : EnumerateAccessOrigins(
                    firstOriginX, firstOriginY, boundsMax, 4);
            foreach (Tile2i corner in terrainColumnOrigins)
            {
            ...
            RelTile1i terrainOnlyClearance = ExtractVehicleClearance(pathParams);
            for (int x = 0; x < terrainOnlyClearance.Value; x++)
            ...
            foreach (Tile2i tile in groundHeight2.Keys)
            {
                ...
                // Cleanup and evaluator preparation consume only this set.
                if (provider.IsPathable(tile, terrainOnlyPathabilityMask))
            ...
            if (CurrentTerrainDesignationRevision != captureDesignationRevision
                && HasTerrainDesignationMutationSince(
                    ...
                    physicalTerrainMax))
            {
                captureDiagnostics.MarkEnvironmentallyDirty(
                    new AccessCaptureRevision(
                        captureRevision.WorldGeneration,
                        CurrentTerrainDesignationRevision,
                        policy.SemanticFingerprint),
                    "TerrainDesignationRevisionChanged");
            }
            ...
            // Reduced snapshots intentionally use Dijkstra. The existing A*
            // potential is a dense enclosing-box array and would defeat sparse
            // corridor memory accounting.
            bool useAStar = policy.UseAStar && reducedDomain == null;
            ...
            if (sliceControl != null)
            {
                ...
                {
                    if (reducedDomain != null
                        && !reducedDomain.CaptureTiles.Contains(occupied))
                        continue;
                ...
                != policy.SemanticFingerprint)
            {
                output.FailureReason = "CaptureInvalidated";
                captureDiagnostics.SetTerminalReason(
                    "CaptureInvalidated:AccessPolicyChanged");
                yield break;
            }
            ...
            long finalEstimate =
                AccessSnapshotMemoryEstimator.EstimateRetainedBytes(
                    groundHeight2.Count,
                    terrainCenterHeight2.Count,
                    terrainColumns.Count,
                    fixedProfiles.Count,
                    workOrigins.Count,
                    groundNodes.Count,
                    towerReachableGround.Count,
                    rayDesignations.Count,
                    projectedDesignationDisturbance.Count,
            ...
            if (!captureBudget.TryAccept(finalEstimate))
            {
                ...
                captureDiagnostics.SetTerminalReason(output.FailureReason);
                LogExperimentalAccessDebug(
                    "[ATD Access Capture] terminal=SnapshotTooLarge phase=final "
                    + captureDiagnostics.Format());
                yield break;
            }
            captureDiagnostics.ObserveCompletion(
                new AccessCaptureRevision(
                    captureRevision.WorldGeneration,
                    CurrentTerrainDesignationRevision,
            ...
            snapshot = new AccessSearchSnapshot(
                ...
                ownedOccupiedTiles
                    ?? (reducedDomain == null
                        ...
                        : buildingFacts.EnumerateOccupiedTiles().Where(
                            reducedDomain.CaptureTiles.Contains)),
                oceanTiles,
                sliceControl != null
                ...
                prebuiltV2FixedNavigationGraph:
                    prebuiltV2FixedNavigationGraph,
                captureCoverage: reducedDomain?.CaptureTiles);
            snapshotTimer.Stop();
            ...
            LogExperimentalAccessDebug(
                ...
                $"rayConservatism={snapshot.Policy.RaySlopeConservatism.ToString("0.##", CultureInfo.InvariantCulture)} " +
                $"rayBuffer={snapshot.Policy.RayEndBuffer} " +
                $"candidateRayMaxDistance={snapshot.Policy.CandidateRayMaxDistance} " +
        ...
        private static List<AccessCapturedProp> CaptureAccessPropFacts(
            IAreaManagingTower tower,
            TerrainManager terrMgr,
            Tile2i boundsMin,
            Tile2i boundsMax,
        ...
        {
            var captured = new List<AccessCapturedProp>();
            if (s_terrainPropsManager != null)
            {
                var area = new RectangleTerrainArea2i(
                    boundsMin,
                    new RelTile2i(
                        boundsMax.X - boundsMin.X + 1,
                        boundsMax.Y - boundsMin.Y + 1));
                var occupiedTiles = new Lyst<Tile2i>();
                IEnumerable<TerrainPropData> props = captureCoverage == null
                    ? s_terrainPropsManager.EnumeratePropsInArea(area)
                    : s_terrainPropsManager.TerrainProps.Values;
                foreach (TerrainPropData prop in props)
                {
                    ...
                    {
                        bool intersectsCoverage = false;
                        for (int tileIndex = 0;
                            tileIndex < occupiedTiles.Count;
                            tileIndex++)
                        {
                            if (!captureCoverage.Contains(
                                    occupiedTiles[tileIndex]))
                                continue;
                            intersectsCoverage = true;
                            break;
                        }
                        if (!intersectsCoverage)
                            continue;
                    }
                    captured.Add(new AccessCapturedProp(
                        isTree: false,
                        isDenseDebris: true,
                        cleanupObjectKey: BuildPropCleanupKey(prop.Id),
                        occupiedTiles: occupiedTiles.ToArray(),
                        ...
                        placedHeight: prop.PlacedAtHeight.Value.ToFloat(),
                        dumpBurialThreshold:
            ...
            if (s_treesManager != null)
            {
                IEnumerable<TreeId> treeIds = captureCoverage == null
                    ? s_treesManager.EnumerateTreesInArea(tower.Area)
                    : s_treesManager.Trees.Keys;
                foreach (TreeId treeId in treeIds)
                {
                    if (!s_treesManager.TryGetTree(treeId, out TreeData tree))
                        continue;
                    if (captureCoverage != null
                        && !captureCoverage.Contains(tree.Id.Position.AsFull))
                        continue;
                    captured.Add(new AccessCapturedProp(
                        isTree: true,
                        isDenseDebris: false,
                        cleanupObjectKey: BuildTreeCleanupKey(treeId),
                        occupiedTiles: new[] { tree.Id.Position.AsFull }));
                }
            }
            return captured;
        }
        ...
        private static Dictionary<Tile2i, AccessPropCleanupInfo> BuildAccessPropCleanupByOrigin(
            ...
            IReadOnlyList<AccessCapturedProp> capturedProps,
            AccessCapturedBuildingFacts buildingFacts,
            ...
            AccessCapturedBuildingFacts buildingFacts)
        {
            if (!IsOriginInsideTower(tower, origin)
                ...
                || buildingFacts.DoesOriginOverlap(origin))
                return false;
            return true;
        }
        ...
        private static AccessPathRequest BuildMergedGoalAccessRequest(
            ...
            IEnumerable<Tile2i>? sourceOriginsOverride = null)
        {
            int requiredWidth = snapshot.VehicleWidth > 4 ? 2 : 1;
            List<Tile2i> sourceOrigins = (sourceOriginsOverride
            ...
            AccessOriginCluster cluster)
        {
            ...
            AccessSearchResult result = AccessPathSearch.FindPath(request, workspace);
            searchTimer.Stop();
                ...
                && !IsCancellationRequested())
            {
                ...
                while (!sessionBuilder.IsComplete
                    ...
                    && (sliceControl == null
                        ...
                        < snapshot.Policy.SearchTimeoutSeconds);
            ...
            AccessSearchPolicySnapshot? policy = null)
        {
            ...
            try
            {
                AccessSearchPolicySnapshot currentPolicy =
                    policy ?? AccessSearchPolicySnapshot.Capture();
                int nodeLimit = currentPolicy.MaxVisitedNodes;
                int timeLimit = currentPolicy.SearchTimeoutSeconds;
            ...
            if (result.Success)
            {
                ...
                if (AtdDiagnostics.IsEnabled(AtdDiagnosticLevel.Debug))
                {
                    ...
                    float selectedCenterOnlyCost = result.Cost
                        - snapshot.Policy.SideRayWeight * selectedSideRayCost;
                    LogExperimentalAccessDebug(
                    ...
                    $"directWeight={snapshot.Policy.DirectWorkWeight.ToString("0.##", CultureInfo.InvariantCulture)} " +
                    $"sideRayWeight={snapshot.Policy.SideRayWeight.ToString("0.##", CultureInfo.InvariantCulture)} " +
                    $"maxRayCost={snapshot.Policy.RayMaxCost.ToString("0.##", CultureInfo.InvariantCulture)} " +
                    $"unresolvedRayCap={snapshot.Policy.RayUnresolvedPenalty.ToString("0.##", CultureInfo.InvariantCulture)} " +
            ...
            AccessSearchWorkspace? workspace = null)
        {
            ...
            for (int index = 1; index < result.Path.Count; index++)
            {
                ...
                IReadOnlyList<AccessGroundHandoff> handoffs =
                    (workspace?.Evaluator ?? EmptyAccessSearchEvaluator.Instance)
                        .GetWorkableHandoffs(
                ...
                if (!AccessHandoffEvaluator.TryGetDirectionalHandoff(
                    ...
                    continue;
                RelTile2i outward =
                    AccessHandoffEvaluator.GetHandoffOutwardDirection(edge);
...
namespace AutoTerrainDesignations.Access
{
    ...
    internal static class AccessHandoffEvaluator
    {
        ...
        private const uint ReadyPerimeterMask = 0x1F8C63F;
            ...
            for (int offset = 0; offset <= 4; offset++)
            {
                ...
                if (vehicleClearance > 0)
                {
                    ...
                    bool secondRankPathable = IsV1PostWorkHandoffTilePathable(
                        origin, profile, terrainHeights, operation, secondRankTile,
                ...
                propCleanupByOrigin = null)
        {
            ...
            int[] handoffSigns = GetEdgeHeightSigns(
                ...
                terrainHeights, collectDeltas: false, out _);
            ...
            for (int offset = 0; offset <= 4; offset++)
            {
                ...
                if (vehicleClearance > 0)
                {
                    ...
                    bool secondRankPathable = IsV1PostWorkHandoffTilePathable(
                        last.Origin, last.Profile, terrainHeights, operation,
            ...
            out uint fulfilledBitmap)
        {
            ...
            return fulfilledBitmap != AllDesignationTilesMask
                && (fulfilledBitmap & ReadyPerimeterMask) != 0;
        }
            ...
            if (operation == AccessHandoffOperation.Mining)
                return terrainPathableWithoutProps.Contains(tile)
                    || IsHandoffWorkTile(origin,
                        CreateDesignationData(origin, profile), terrainHeights,
                        operation, tile);
            if (operation == AccessHandoffOperation.Dumping)
            {
            ...
            Tile2i tile)
        {
            ...
            if (!terrainHeights.TryGetValue(tile, out float groundHeight))
                return false;
        ...
        internal static RelTile2i GetHandoffOutwardDirection(int handoffEdge)
            => handoffEdge == 0
                ? new RelTile2i(-1, 0)
                : handoffEdge == 1
                    ? new RelTile2i(1, 0)
            ...
            out string diagnostic)
        {
            ...
            return TrySelectV2CornerCrestHandoff(
                ...
                handoffEdge, terrainHeights, bridgeTilePathable,
        ...
        internal static bool TrySelectV2CornerCrestHandoff(
        ...
        internal static bool TryGetDirectionalHandoff(
            Tile2i origin,
            AccessHeightProfile profile,
            Tile2i predecessorPosition,
            IReadOnlyDictionary<Tile2i, float> terrainHeights,
            ...
            out string diagnostic)
        {
            ...
            int[] handoffSigns = GetEdgeHeightSigns(
                origin, profile, handoffEdge, terrainHeights,
                false, out _);
            ...
            if (handoffSigns.All(sign => sign == 0)
                && IsProfileExactTerrain(origin, profile, terrainHeights))
            {
                operation = AccessHandoffOperation.None;
                fulfilledBitmap = AllDesignationTilesMask;
                return true;
            }
            ...
            out uint fulfilledBitmap)
        {
            int[] incomingSigns = GetEdgeHeightSigns(
                origin, profile, OppositeEdge(handoffEdge), terrainHeights,
            ...
            IReadOnlyDictionary<Tile2i, float> terrainHeights)
        {
            ...
            for (int offset = 0; offset <= 4; offset++)
            {
                ...
                int y = edge == 2 ? 0 : edge == 3 ? 4 : offset;
                Tile2i tile = origin + new RelTile2i(x, y);
                heights[offset] = terrainHeights.TryGetValue(
                    tile, out float groundHeight)
                    ? groundHeight : 0f;
            }
            return heights;
        }
        ...
        internal static bool TryGetConnectedAndHandoffCorners(
            Tile2i origin,
            Tile2i predecessorPosition,
            out int connectedA,
            out int connectedB,
            ...
            Tile2i tile)
        {
            if (groundNodes.Contains(tile))
                return true;
            return propCleanupByOrigin.TryGetValue(
                    GetDesignationOrigin(tile), out AccessPropCleanupInfo info)
                && info.IsEligible
        ...
        private static HeightTilesF GetDesignationTargetHeightAt(
            DesignationData data,
            int x,
            int y)
        {
            HeightTilesF west = data.OriginTargetHeight.HeightTilesF.Lerp(
                data.PlusYTargetHeight.HeightTilesF, y, 4);
            HeightTilesF east = data.PlusXTargetHeight.HeightTilesF.Lerp(
                data.PlusXyTargetHeight.HeightTilesF, y, 4);
            return west.Lerp(east, x, 4);
        }

        private static int ToHeight2(float height)
            => (int)Math.Round(height * 2f);

        private static Tile2i GetDesignationOrigin(Tile2i tile)
            => new Tile2i(
                (int)Math.Floor(tile.X / 4.0) * 4,
                (int)Math.Floor(tile.Y / 4.0) * 4);
    ...
    public static partial class AutoDepthDesignation
    {
        ...
        private static IReadOnlyDictionary<Tile2i, float>
            CaptureAccessHandoffTerrainHeights(
                TerrainManager terrMgr,
                params Tile2i[] origins)
        {
            var heights = new Dictionary<Tile2i, float>();
            foreach (Tile2i origin in origins)
                for (int y = 0; y <= 4; y++)
                    for (int x = 0; x <= 4; x++)
                    {
                        Tile2i tile = origin + new RelTile2i(x, y);
                        if (!heights.ContainsKey(tile))
                            heights[tile] = terrMgr.GetHeight(tile)
                                .Value.ToFloat();
                    }
            return heights;
        }
            ...
            out string reason)
        {
                ...
                out string laneReason)
            {
                ...
                if (!TryGetV2RouteProfile(
                    ...
                    || !AccessHandoffEvaluator.TryGetConnectedAndHandoffCorners(
                        terminalOrigin,
                        new Tile2i(
            ...
            out string failureReason)
        {
            ...
            var terrainWork = plannedPlacements.Values.Select(planned =>
                new AccessPropPlannedTerrainWork(planned.Data,
                    planned.Proto == s_miningProto ? AccessHandoffOperation.Mining
                        : planned.Proto == s_dumpingProto ? AccessHandoffOperation.Dumping
                        : planned.Proto == s_levelingProto ? AccessHandoffOperation.Leveling
                        : AccessHandoffOperation.None)).ToArray();
            var projectedWork = new Dictionary<Tile2i, AccessProjectedTerrainEffect>();
            AccessPathSearch.BuildFinalGeneratedDisturbedTiles(
                snapshot, searchResult, projectedWork: projectedWork);
                ...
                in cleanupOriginsByProp)
            {
                ...
                var liveSample = new AccessPropSample(sample.Tile, false, true, true,
                    dumpBurialProbeTile: liveProp.Position.Tile2i,
                    dumpBurialProbeOffsetX: liveProp.PositionWithinTile.X.ToFloat(),
                    dumpBurialProbeOffsetY: liveProp.PositionWithinTile.Y.ToFloat(),
                    placedHeight: liveProp.PlacedAtHeight.Value.ToFloat(),
                    dumpBurialThreshold: burialThreshold);

                QuickRemoveDebrisPolicy policy =
                    AccessQuickRemoveDebrisPolicy;
                if (!AccessPropTerrainRemoval.TryGetRemovalStrategy(
                        policy, liveSample,
                        liveProp.PlacementHeightOffset.Value.ToFloat(),
                        terrainWork,
                        tile => projectedWork.TryGetValue(tile, out AccessProjectedTerrainEffect effect)
                            ? effect : default,
            ...
            AccessTileCoverage? captureCoverage = null)
        {
            ...
            if (!TryFindNearestTowerGroundSeed(
                    tower, groundNodes, provider, pathParams,
                    towerAccessPosition, out start, captureCoverage))
                return false;
        ...
        private static bool TryFindNearestTowerGroundSeed(
            ...
            AccessTileCoverage? captureCoverage = null)
        {
            if ((captureCoverage == null || captureCoverage.Contains(origin))
                && IsTowerGroundSeed(
                    tower, groundNodes, provider, pathParams, origin))
            {
                seed = origin;
                return true;
            }
            ...
            for (int radius = 1; radius <= RAMP_ACCESS_SEARCH_MARGIN_TILES; radius++)
            {
                ...
                {
                    if (TryUseTowerGroundSeed(tower, groundNodes, provider, pathParams, origin + new RelTile2i(-radius, y), out seed, captureCoverage)
                        || TryUseTowerGroundSeed(tower, groundNodes, provider, pathParams, origin + new RelTile2i(radius, y), out seed, captureCoverage))
                        return true;
                }

                for (int x = -radius + 1; x < radius; x++)
                {
                    if (TryUseTowerGroundSeed(tower, groundNodes, provider, pathParams, origin + new RelTile2i(x, -radius), out seed, captureCoverage)
                        || TryUseTowerGroundSeed(tower, groundNodes, provider, pathParams, origin + new RelTile2i(x, radius), out seed, captureCoverage))
                        return true;
                }
            }
        ...
        private static bool TryUseTowerGroundSeed(
            ...
            AccessTileCoverage? captureCoverage = null)
        {
            if ((captureCoverage == null || captureCoverage.Contains(candidate))
                && IsTowerGroundSeed(
                    tower, groundNodes, provider, pathParams, candidate))
            {
                seed = candidate;
                return true;
            }
        ...
        private static IEnumerable<Tile2i> EnumerateAccessRectangle(
            Tile2i min, Tile2i max)
        {
        ...
        private static bool CoverageIntersectsDesignation(
            AccessTileCoverage coverage, Tile2i origin)
        {
            for (int y = 0; y <= 4; y++)
                for (int x = 0; x <= 4; x++)
                    if (coverage.Contains(origin + new RelTile2i(x, y)))
                        return true;
            return false;
        }

        private static IEnumerable<Tile2i> EnumerateAccessOrigins(
            int firstX, int firstY, Tile2i max, int extra)
        {
            for (int x = firstX; x <= max.X + extra; x += 4)
                for (int y = firstY; y <= max.Y + extra; y += 4)
                    yield return new Tile2i(x, y);
        }

        private static IEnumerable<Tile2i> EnumerateRequiredTerrainColumns(
            AccessTileCoverage searchOrigins)
        {
            var columns = new HashSet<Tile2i>();
            foreach (Tile2i origin in searchOrigins.EnumerateTiles())
            {
                columns.Add(origin);
                columns.Add(origin + new RelTile2i(4, 0));
                columns.Add(origin + new RelTile2i(0, 4));
                columns.Add(origin + new RelTile2i(4, 4));
            }
            foreach (Tile2i column in columns
