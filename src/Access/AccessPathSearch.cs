...
namespace AutoTerrainDesignations.Access
{
    internal static class AccessPathSearch
    {
        ...
        public static bool ValidateCoreTransitions(out string failure)
        {
            ...
            var noFeasibleV2StartResult = new AccessSearchResult(
                false, "V2NoFeasibleStart", Tile2i.Zero,
                Array.Empty<AccessSearchNode>(), 0f, 0,
                new Dictionary<string, int>());
            string[] interruptedReasons =
            {
                "SearchTimeLimit",
                "SearchCancelled",
                "VisitedLimit",
            ...
            if (!noPathResult.SearchSpaceExhausted
                || !noFeasibleV2StartResult.SearchSpaceExhausted
                || interruptedReasons.Any(reason =>
                    new AccessSearchResult(
                        false, reason, Tile2i.Zero,
                        Array.Empty<AccessSearchNode>(), 0f, 0,
                        new Dictionary<string, int>())
                    .SearchSpaceExhausted))
            {
                failure = "outside-area fallback must retry after a drained or empty in-area frontier, never after a search interruption";
                return false;
            }
            ...
            Tile2i fixtureDirectEdgeGround = new Tile2i(0, 1);
            var fixture = new AccessSearchSnapshot(
                ...
                new[] { fixtureGoal, fixtureDirectEdgeGround },
                new[] { fixtureGoal },
                Array.Empty<Tile2i>(),
                Array.Empty<Tile2i>(),
            ...
            if (!TryGetHandoffAggressionRank(
                    AccessSearchMode.XPositive, new Tile2i(4, 0),
                    AccessHandoffOperation.Mining, out int miningUpRank)
                || !TryGetHandoffAggressionRank(
                    AccessSearchMode.Flat, new Tile2i(4, 0),
                    AccessHandoffOperation.Mining, out int miningFlatRank)
                || !TryGetHandoffAggressionRank(
                    AccessSearchMode.XNegative, new Tile2i(4, 0),
                    AccessHandoffOperation.Mining, out int miningDownRank)
                || miningUpRank != 0 || miningFlatRank != 1
                || miningDownRank != 2
                || !TryGetHandoffAggressionRank(
                    AccessSearchMode.XNegative, new Tile2i(4, 0),
                    AccessHandoffOperation.Dumping, out int dumpingDownRank)
                || !TryGetHandoffAggressionRank(
                    AccessSearchMode.Flat, new Tile2i(4, 0),
                    AccessHandoffOperation.Dumping, out int dumpingFlatRank)
                || !TryGetHandoffAggressionRank(
                    AccessSearchMode.XPositive, new Tile2i(4, 0),
                    AccessHandoffOperation.Dumping, out int dumpingUpRank)
            ...
            var miningUp = new AccessSearchNode(
                new Tile2i(8, 4), 1, AccessSearchMode.XPositive,
                entryDirection: new Tile2i(4, 0));
            var miningFlat = new AccessSearchNode(
                new Tile2i(8, 4), 0, AccessSearchMode.Flat,
                entryDirection: new Tile2i(4, 0));
            var dominanceHandoff = new AccessGroundHandoff(
                new Tile2i(13, 6), AccessHandoffOperation.Mining,
                new[] { new Tile2i(12, 6), new Tile2i(13, 6) }, 1);
            if (!dominanceCache.RecordSuccess(
                    dominanceParent, miningUp, dominanceHandoff, 10f)
                || !dominanceCache.IsDominated(
                    dominanceParent, miningFlat, dominanceHandoff, 11f)
                || dominanceCache.IsDominated(
                    dominanceParent, miningFlat, dominanceHandoff, 9f)
                || dominanceCache.IsDominated(
                    dominanceParent, miningFlat,
                    new AccessGroundHandoff(
                        new Tile2i(13, 7), AccessHandoffOperation.Mining,
                        dominanceHandoff.EscapeTiles, 1), 11f))
            { failure = "V1 handoff dominance must require cheaper success and exact exit geometry"; return false; }
            ...
            AccessV1LevelingEdgeSeed[] xSeeds =
                EnumerateV1LevelingEdgeSeeds(new Tile2i(8, 1)).ToArray();
            AccessV1LevelingEdgeSeed[] negativeXSeeds =
                EnumerateV1LevelingEdgeSeeds(new Tile2i(-8, -2)).ToArray();
            AccessV1LevelingEdgeSeed[] ySeeds =
                EnumerateV1LevelingEdgeSeeds(new Tile2i(1, 8)).ToArray();
            AccessV1LevelingEdgeSeed[] excludedSeeds =
                EnumerateV1LevelingEdgeSeeds(new Tile2i(7, 7)).ToArray();
            if (xSeeds.Length != 2
                || !xSeeds.Any(seed => seed.Origin == new Tile2i(8, 0)
                    && seed.OutwardDirection == new Tile2i(-4, 0))
                || !xSeeds.Any(seed => seed.Origin == new Tile2i(4, 0)
                    && seed.OutwardDirection == new Tile2i(4, 0))
                || negativeXSeeds.Length != 2
                || !negativeXSeeds.Any(seed => seed.Origin == new Tile2i(-8, -4)
                    && seed.OutwardDirection == new Tile2i(-4, 0))
                || !negativeXSeeds.Any(seed => seed.Origin == new Tile2i(-12, -4)
                    && seed.OutwardDirection == new Tile2i(4, 0))
                || ySeeds.Length != 2
                || !ySeeds.Any(seed => seed.Origin == new Tile2i(0, 8)
                    && seed.OutwardDirection == new Tile2i(0, -4))
                || !ySeeds.Any(seed => seed.Origin == new Tile2i(0, 4)
                    && seed.OutwardDirection == new Tile2i(0, 4))
                || excludedSeeds.Length != 0)
            { failure = "V1 leveling edge seeds must be sign-symmetric"; return false; }
            bool foundDirectWest = TryGetV1DirectLevelingEdgeHandoff(
                fixture, Tile2i.Zero, flat, fixtureDirectEdgeGround,
                out Tile2i directWest);
            bool foundRaisedDirect = TryGetV1DirectLevelingEdgeHandoff(
                fixture, Tile2i.Zero, raisedFlat, fixtureDirectEdgeGround,
                out _);
            if (!foundDirectWest
                || directWest != new Tile2i(-4, 0)
                || foundRaisedDirect)
            {
                failure = "V1 direct leveling must use a level shared edge: "
                    + $"flat={foundDirectWest}/{directWest} raised={foundRaisedDirect}";
                return false;
            }
            ...
            float creditedDumpingInterior = EstimateDirectWorkCost(
                fixture, new Tile2i(12, 12), raisedFlat,
                AccessHandoffOperation.Dumping,
                _ => new AccessProjectedTerrainEffect
                {
                    HasFillWork = true,
                    FillFloor = 0.5f,
                });
            if (Math.Abs(baselineLandscapingCost.DirectWorkCost - 16f) > 0.0001f
                ...
                || Math.Abs(creditedDumpingInterior - 8f) > 0.0001f
            ...
            AccessSideRayResult dryOceanCutRay = AccessSideRayCost.Score(
                ...
                Tile2i.Zero, new Tile2i(1, 0), 0f,
                AccessSideRayOperation.Cut, 1f);
            AccessSideRayResult sameSortTerminationRay =
                AccessSideRayCost.Score(
                    tile => new AccessSideRayTerrainSample(
                        AccessTerrainSampleKind.Terrain,
                        10f,
                        terminatesSameOperationRay: tile.X == 3),
                    Tile2i.Zero, new Tile2i(1, 0), 0f,
                    AccessSideRayOperation.Cut, 1f);
            AccessSideRayResult heightAwareTerminationRay =
                AccessSideRayCost.Score(
                    tile => new AccessSideRayTerrainSample(
                        AccessTerrainSampleKind.Terrain,
                        tile.X == 3 ? 3f : 10f),
                    ...
                    AccessSideRayOperation.Cut, 1f,
                    postTerminationSafetyMargin: 2);
            AccessSideRayResult heightAwareContinuationRay =
                AccessSideRayCost.Score(
                    tile => new AccessSideRayTerrainSample(
                        AccessTerrainSampleKind.Terrain,
                        tile.X == 3 || tile.X == 5 ? 3f : 10f),
                    Tile2i.Zero, new Tile2i(1, 0), -2f,
                    AccessSideRayOperation.Cut, 1f,
                    maxTraceDistance: 5);
            if (fillOceanRay.IsFatal
                ...
                || sameSortTerminationRay.IsUnresolved
                || sameSortTerminationRay.IsFatal
                || sameSortTerminationRay.DisturbedDistance != 3
                || sameSortTerminationRay.SampleCount != 3
                || Math.Abs(sameSortTerminationRay.TotalCost - 17f) > 0.0001f
                || heightAwareTerminationRay.IsFatal
                || heightAwareTerminationRay.IsUnresolved
                || Math.Abs(heightAwareTerminationRay.TotalCost - 17f) > 0.0001f
                || heightAwareTerminationRay.DisturbedDistance != 5
                || !heightAwareTerminationRay.IsWorkDistance(1)
                || !heightAwareTerminationRay.IsWorkDistance(2)
                || !heightAwareTerminationRay.IsSafetyOnlyDistance(3)
                || !heightAwareTerminationRay.IsSafetyOnlyDistance(5)
                || heightAwareTerminationRay.IsWorkDistance(3)
                || heightAwareContinuationRay.IsFatal
                || heightAwareContinuationRay.IsUnresolved
                || Math.Abs(heightAwareContinuationRay.TotalCost - 23f) > 0.0001f
                || !heightAwareContinuationRay.IsWorkDistance(3)
                || !heightAwareContinuationRay.IsWorkDistance(4)
                || !heightAwareContinuationRay.IsSafetyOnlyDistance(5))
            ...
            if (IsCompatibleWithPathHistory(
                    null, historyOrigin, highFlat, disturbanceHistory, null,
            ...
            if (IsCompatibleWithPathHistory(
                    null, adjacentHistoryOrigin, highFlat, disturbanceHistory, null,
            ...
            if (!IsCompatibleWithPathHistory(
                    null, adjacentHistoryOrigin, highFlat, disturbanceHistory, historyOrigin,
                    out _))
            { failure = "generated history must allow the immediate predecessor edge"; return false; }
            if (!IsCompatibleWithPathHistory(
                    null, historyOrigin + new RelTile2i(4, 4), highFlat,
            ...
            GeneratedPathHistory sameSortCutEnvelopeHistory =
                GeneratedPathHistory.Empty.WithGenerated(
                    historyOrigin, highFlat, Array.Empty<Tile2i>(),
                    rayHeightConstraints: new[]
                    {
                        new AccessRayHeightConstraint(
                            rayEnvelopeTile, AccessSideRayOperation.Cut, -5f),
                    });
            GeneratedPathHistory sameSortFillEnvelopeHistory =
                GeneratedPathHistory.Empty.WithGenerated(
                    historyOrigin, highFlat, Array.Empty<Tile2i>(),
                    rayHeightConstraints: new[]
                    {
                        new AccessRayHeightConstraint(
                            rayEnvelopeTile, AccessSideRayOperation.Fill, 5f),
                    });
            ...
            if (sameSortCutEnvelopeHistory.IsProfileBlockedByRayEnvelope(
                    directionalSnapshot, new Tile2i(16, 8), lowFlat, out _)
                || sameSortFillEnvelopeHistory.IsProfileBlockedByRayEnvelope(
                    directionalSnapshot, new Tile2i(16, 8), highFlat, out _))
            { failure = "V1 cut rays must merge with new cut work and fill rays with new fill work"; return false; }
            Tile2i mergeRayTile = new Tile2i(20, 6);
            GeneratedPathHistory mergeRayHistory =
                GeneratedPathHistory.Empty.WithGenerated(
                    historyOrigin, highFlat, Array.Empty<Tile2i>(),
                    rayHeightConstraints: new[]
                    {
                        new AccessRayHeightConstraint(
                            mergeRayTile, AccessSideRayOperation.Cut, -2f),
                    });
            AccessSideRayResult mergedV1Ray = ScoreExitCorner(
                directionalSnapshot,
                new Tile2i(20, 8), -4f, new Tile2i(0, -1),
                AccessHandoffOperation.Leveling,
                terminatesAtSameOperationRay:
                    (tile, operation) =>
                        mergeRayHistory.HasRayAt(tile, operation));
        ...
        public static AccessPathSearchSession CreateSession(AccessPathRequest request)
        {
            ...
            if (request.RequiredWidth == 2)
            {
                ...
                diagnostics.V2PotentialBuildTicks = v2Potential != null
                    ...
                    : 0;
                diagnostics.V2PotentialGeneratedNodes =
            ...
            float maxCostLimit)
        {
            ...
            var handoffDominance = new AccessV1HandoffDominanceCache();
        ...
        public sealed class AccessPathSearchSession
        {
            ...
            private readonly AccessV1HandoffDominanceCache m_handoffDominance;
                ...
                AccessV1HandoffDominanceCache handoffDominance)
            {
                ...
                m_handoffDominance = handoffDominance;
            ...
            public int Step(int maxVisitedNodes)
            {
                ...
                while (m_queue.Count > 0 && m_visited < MaxVisitedNodes && visitedThisStep < maxVisitedNodes)
                {
                    ...
                    if (current.IsGround)
                    {
                        if (m_includeGroundGoals
                            && m_snapshot.UseAStar
                            && m_useAStarHeuristic
                            && TryBuildGroundGoalSuffix(
                                current, known,
                                out AccessSearchResult suffixCandidate))
                        {
                            ...
                            string suffixFailure = suffixPlan.IsValid
                                ? m_rejectGoal?.Invoke(suffixCandidate)
                                    ?? string.Empty
                                : string.IsNullOrEmpty(suffixPlan.FailureReason)
                                    ? "Materialization"
                                    : suffixPlan.FailureReason;
                            if (string.IsNullOrEmpty(suffixFailure))
                            {
                                m_diagnostics.V1GroundSuffixSuccesses++;
                                m_diagnostics.GoalAcceptedAtVisited = m_visited;
                                Result = suffixCandidate;
                                IsComplete = true;
                                return visitedThisStep;
                            }

                            m_diagnostics.GoalRejected++;
                            m_diagnostics.V1GroundSuffixFallbacks++;
                            Reject(m_rejections, "Goal" + suffixFailure);
                            m_lastRejectedGoalPath =
                                suffixCandidate.Path.ToList();
            ...
            private bool TryBuildGroundGoalSuffix(
                AccessSearchNode current,
                float currentCost,
                out AccessSearchResult candidate)
            {
                candidate = null!;
                AccessV1GroundGoalDistance? field =
                    m_snapshot.V1GroundGoalDistance;
                if (field == null
                    || !field.TryGetDistance(
                        current.Position, out float remaining)
                    || remaining <= 0.0001f)
                    return false;

                m_diagnostics.V1GroundSuffixAttempts++;
                if (!m_generatedHistory.TryGetValue(
                        current, out GeneratedPathHistory history))
                    history = GeneratedPathHistory.Empty;
                var path = Reconstruct(current, m_startNode, m_previous);
                AccessSearchNode cursor = current;
                float cost = currentCost;
                int incomingX = 0;
                int incomingY = 0;
                if (m_previous.TryGetValue(
                        current, out AccessSearchNode parent)
                    && parent.IsGround)
                {
                    incomingX = Math.Sign(
                ...
                int maxSteps = Math.Max(1, field.ReachableNodeCount);
                for (int step = 0; step < maxSteps; step++)
                {
                    if (m_snapshot.IsGoalGroundNode(cursor.Position))
                    {
                        candidate = BuildResult(
                            true, string.Empty, m_startOrigin, m_startNode,
                            path, cost, m_visited, m_rejections, m_snapshot,
                            AccessReachedGoalKind.TowerGround, m_diagnostics);
                        return true;
                    }

                    bool advanced = false;
                    foreach (Tile2i nextTile in
                        field.EnumerateDescendingSteps(cursor.Position))
                    {
                        int dx = Math.Sign(nextTile.X - cursor.Position.X);
                        int dy = Math.Sign(nextTile.Y - cursor.Position.Y);
                        if (incomingX * dx + incomingY * dy < 0)
                            continue;
                        bool nextIsGround = m_snapshot.IsGroundNode(nextTile);
                        bool nextIsCleanup = !nextIsGround
                            && m_snapshot.CanTraverseToCleanupGround(
                                cursor.Position, nextTile);
                        if (!nextIsGround && !nextIsCleanup)
                            continue;
                        if (dx != 0 && dy != 0)
                        {
                            Tile2i sideX = new Tile2i(
                                nextTile.X, cursor.Position.Y);
                            Tile2i sideY = new Tile2i(
                                cursor.Position.X, nextTile.Y);
                            if (!m_snapshot.IsGroundNode(sideX)
                                || !m_snapshot.IsGroundNode(sideY)
                                || history.IsGroundDisturbed(sideX)
                                || history.IsGroundDisturbed(sideY))
                                continue;
                        }
                        if (history.IsGroundDisturbed(nextTile)
                            || !m_snapshot.TryGetGroundHeight2(
                                nextTile, out int height2))
                            continue;

                        var next = new AccessSearchNode(
                            nextTile, height2, AccessSearchMode.Ground);
                        float stepCost = dx != 0 && dy != 0
                            ? GroundDiagonalCost : 1f;
                        float nextCost = cost + stepCost
                            + GetCleanupEntryCost(
                                m_snapshot, cursor.Position, nextTile);
                        if (nextCost > m_maxCostLimit + 0.0001f)
                            continue;
                        ...
                        incomingX = dx;
                        incomingY = dy;
                        m_diagnostics.V1GroundSuffixSteps++;
                        advanced = true;
                        break;
                    }
            ...
            AccessV1HandoffDominanceCache handoffDominance)
        {
            ...
            foreach (AccessGroundHandoff handoff in handoffs)
            {
                ...
                float finalHandoffCost = handoffBaseCost
                    + Manhattan(current.CostPosition, handoff.Tile)
                    + handoffCleanupCost;
                if (previous.TryGetValue(
                        current, out AccessSearchNode dominanceParent)
                    && !dominanceParent.IsGround
                    && handoffDominance.IsDominated(
                        dominanceParent, current, handoff,
                        finalHandoffCost))
                {
                    diagnostics.V1HandoffDominancePrunes++;
                    continue;
                }
                if (traceFirstGeneratedHandoff)
                    diagnostics.RecordFirstGeneratedHandoff(
                        ...
                        $"acceptedCost={finalHandoffCost.ToString("0.##", System.Globalization.CultureInfo.InvariantCulture)}");
                ...
                if (previous.TryGetValue(
                        current, out dominanceParent)
                    && !dominanceParent.IsGround
                    && handoffDominance.RecordSuccess(
                        dominanceParent, current, handoff,
                        finalHandoffCost))
                    diagnostics.V1HandoffDominanceSuccesses++;
            }
            ...
            void AddForwardTerminalHandoffs()
            {
                    ...
                    GeneratedPathHistory provisionalHistory)
                {
                    ...
                    foreach (AccessSearchMode mode in modes)
                    {
                        ...
                        if (!IsCompatibleWithPathHistory(
                                snapshot, nextOrigin, nextProfile, provisionalHistory,
            ...
            foreach (AccessSearchMode mode in s_vModes)
            {
                ...
                bool historyCompatible = !hasCurrent
                    || IsCompatibleWithPathHistory(
                        snapshot, nextOrigin, nextProfile, current, generatedHistory,
                        out historyReason);
                ...
                phaseStart = AtdDiagnostics.Timestamp();
                if (!generatedHistory.TryGetValue(
                        current, out GeneratedPathHistory currentRayHistory))
                    currentRayHistory = GeneratedPathHistory.Empty;
                ...
                AccessSideRayResult turnOuterRay = ScoreTurnOuterCorner(
                    ...
                    (tile, rayOperation) =>
                        currentRayHistory.HasRayAt(tile, rayOperation),
            ...
            AccessSearchDiagnostics diagnostics)
        {
            ...
            var directProfiles = new HashSet<AccessV1GroundToVProfileKey>();
            if (snapshot.TryGetPreciseTerrainHeight(
                    current.Position, out float directGroundHeight))
            {
                int directLevel = (int)Math.Round(directGroundHeight);
                if (Math.Abs(directGroundHeight - directLevel) <= 0.0001f)
                foreach (AccessV1LevelingEdgeSeed seed in
                    EnumerateV1LevelingEdgeSeeds(current.Position))
                {
                    Tile2i origin = seed.Origin;
                    ...
                    foreach (AccessSearchMode mode in s_vModes)
                    {
                        Tile2i travel = new Tile2i(
                            -seed.OutwardDirection.X,
                            -seed.OutwardDirection.Y);
                        if (!TryProfileAtEntryLevel(
                                mode, directLevel, travel,
                                out AccessHeightProfile profile))
                            continue;
                        if (!TryGetV1DirectLevelingEdgeHandoff(
                                snapshot, origin, profile,
                                current.Position,
                                out Tile2i directEntryDirection)
                            || directEntryDirection
                                != seed.OutwardDirection)
                            continue;
                        var key = new AccessV1GroundToVProfileKey(
                            origin, profile);
                        if (!directProfiles.Add(key))
                            continue;
                        EvaluateProfile(
                            origin, mode, profile,
                            directLeveling: true,
            ...
            foreach (Tile2i origin in
                CandidateOriginsAtGroundTile(current.Position))
            {
                diagnostics.GroundToGeneratedOriginChecks++;
                if (!snapshot.IsOriginInside(origin))
                {
                ...
                foreach (AccessSearchMode mode in s_vModes)
                {
                    ...
                    for (int delta = -3; delta <= 3; delta++)
                    {
                        ...
                        if (directProfiles.Contains(
                                new AccessV1GroundToVProfileKey(origin, profile)))
                            continue;
                        EvaluateProfile(
                            origin, mode, profile,
                            directLeveling: false, default);
                    }
                }
            }
                ...
                Tile2i directEntryDirection)
            {
                ...
                if (directLeveling)
                {
                    handoffOperation = AccessHandoffOperation.Leveling;
                    entryDirection = directEntryDirection;
                    var directNode = new AccessSearchNode(
                        origin, profile.Center2, mode,
                        handoffOperation, entryDirection);
                    float directLowerBound = currentCost
                        + Manhattan(current.Position, directNode.CostPosition);
                    if (IsKnownDistanceNoWorse(
                            distance, directNode, directLowerBound))
                        return;
                }
                diagnostics.GeneratedProfileFeasibleChecks++;
                long phaseStart = AtdDiagnostics.Timestamp();
                ...
                bool historyCompatible = IsCompatibleWithPathHistory(
                    snapshot, origin, profile, current, generatedHistory,
                    out string historyReason);
                ...
                var next = new AccessSearchNode(
                    origin, profile.Center2, mode,
                    handoffOperation, entryDirection);
                float lowerBoundCost = currentCost
                    + Manhattan(current.Position, next.CostPosition);
                if (IsKnownDistanceNoWorse(distance, next, lowerBoundCost))
                    return;
                diagnostics.SideRayCostChecks++;
                phaseStart = AtdDiagnostics.Timestamp();
                ...
                diagnostics.GeneratedRelaxations++;
                if (directLeveling)
                    diagnostics.V1GroundToVDirectLevelingAccepts++;
            ...
            out string reason)
        {
            ...
            return IsCompatibleWithPathHistory(
                snapshot, nextOrigin, nextProfile,
                history, allowedEdgeNeighbor, out reason);
        }
            ...
            out string rejection)
        {
            ...
            if (!envelope.IsV1CenterHeightUseful(
                    center, checked(profile.Center2 * 16), out rejection))
            {
                if (rejection == "HeightEnvelopeAbove")
                    diagnostics.HeightEnvelopeAboveRejections++;
                else if (rejection == "HeightEnvelopeBelow")
        ...
        internal readonly struct AccessV1LevelingEdgeSeed
        {
            public Tile2i Origin { get; }
            public Tile2i OutwardDirection { get; }

            public AccessV1LevelingEdgeSeed(
                Tile2i origin,
        ...
        /// <summary>
        /// Derives both V1 cells adjacent to a canonical grid-edge ground
        /// center. Face orientation selects the side of the shared edge; the
        /// transverse center must remain in V1's middle lanes one or two.
        /// </summary>
        internal static IEnumerable<AccessV1LevelingEdgeSeed>
            EnumerateV1LevelingEdgeSeeds(Tile2i groundTile)
        {
            int x = groundTile.X & 3;
            int y = groundTile.Y & 3;

            if (x == 0 && (y == 1 || y == 2))
            {
                int originY = groundTile.Y - y;
                yield return new AccessV1LevelingEdgeSeed(
                    new Tile2i(groundTile.X, originY),
                    new Tile2i(-4, 0));
                yield return new AccessV1LevelingEdgeSeed(
                    new Tile2i(groundTile.X - 4, originY),
                    new Tile2i(4, 0));
            }
            if (y == 0 && (x == 1 || x == 2))
            {
                int originX = groundTile.X - x;
                yield return new AccessV1LevelingEdgeSeed(
                    new Tile2i(originX, groundTile.Y),
                    new Tile2i(0, -4));
                yield return new AccessV1LevelingEdgeSeed(
                    new Tile2i(originX, groundTile.Y - 4),
                    new Tile2i(0, 4));
            }
        }

        private static IEnumerable<Tile2i> CandidateOriginsAtGroundTile(
            Tile2i tile)
        {
            ...
            Tile2i[] candidates =
            {
                new Tile2i(baseX, baseY),
                new Tile2i(baseX - 4, baseY),
                new Tile2i(baseX, baseY - 4),
                new Tile2i(baseX - 4, baseY - 4),
        ...
        private readonly struct AccessV1GroundToVProfileKey :
            IEquatable<AccessV1GroundToVProfileKey>
        {
            private readonly Tile2i m_origin;
            private readonly int m_nw2;
            private readonly int m_ne2;
            private readonly int m_se2;
            private readonly int m_sw2;

            public AccessV1GroundToVProfileKey(
                Tile2i origin,
                AccessHeightProfile profile)
            {
                m_origin = origin;
            ...
            public bool Equals(AccessV1GroundToVProfileKey other)
            ...
            public override bool Equals(object? obj)
                => obj is AccessV1GroundToVProfileKey other
                    && Equals(other);
        ...
        /// <summary>
        /// Remembers fully accepted V-to-G exits for sibling slope profiles.
        /// The geometry is part of the key: success on one exit never claims
        /// that another exit is pathable. It only suppresses a strictly more
        /// aggressive cut/fill for the same already-proved exit.
        /// </summary>
        internal sealed class AccessV1HandoffDominanceCache
        {
            private readonly Dictionary<Key, Success> m_best =
                new Dictionary<Key, Success>();

            public bool IsDominated(
                AccessSearchNode parent,
                AccessSearchNode current,
                AccessGroundHandoff handoff,
                float candidateCost)
            {
                if (!TryGetHandoffAggressionRank(
                        current.Mode, current.EntryDirection,
                        handoff.Operation, out int rank))
                    return false;
                return m_best.TryGetValue(
                        new Key(parent, current.Position, handoff),
                        out Success best)
                    && best.Rank < rank
                    && best.Cost <= candidateCost + 0.0001f;
            }
                ...
                float cost)
            {
                if (!TryGetHandoffAggressionRank(
                        ...
                        handoff.Operation, out int rank))
                    return false;
                var key = new Key(parent, current.Position, handoff);
                if (m_best.TryGetValue(key, out Success best)
                    && best.Rank <= rank
                    && best.Cost <= cost + 0.0001f)
                    return false;
                m_best[key] = new Success(rank, cost);
                return true;
            }
            ...
            private readonly struct Success
            {
                ...
                public Success(int rank, float cost)
                {
                    Rank = rank;
                    Cost = cost;
                }
            ...
            {
                private readonly AccessSearchNode m_parent;
                private readonly Tile2i m_origin;
                private readonly Tile2i m_groundEntry;
                private readonly AccessHandoffOperation m_operation;
                private readonly int m_spanLength;
                private readonly IReadOnlyList<Tile2i> m_escapeTiles;
                    ...
                    AccessGroundHandoff handoff)
                {
                    m_parent = parent;
                    m_origin = origin;
                    m_groundEntry = handoff.Tile;
                    m_operation = handoff.Operation;
                    m_spanLength = handoff.SpanLength;
                    m_escapeTiles = handoff.EscapeTiles;
                }

                public bool Equals(Key other)
                    => m_parent.Equals(other.m_parent)
                        && m_origin == other.m_origin
                        && m_groundEntry == other.m_groundEntry
                        && m_operation == other.m_operation
                        && m_spanLength == other.m_spanLength
                        && m_escapeTiles.SequenceEqual(other.m_escapeTiles);
                ...
                public override int GetHashCode()
                {
                    unchecked
                    {
                        int hash = m_parent.GetHashCode();
                        hash = (hash * 397) ^ m_origin.GetHashCode();
                        hash = (hash * 397) ^ m_groundEntry.GetHashCode();
                        hash = (hash * 397) ^ (int)m_operation;
                        hash = (hash * 397) ^ m_spanLength;
                        for (int index = 0; index < m_escapeTiles.Count; index++)
                            hash = (hash * 397)
                                ^ m_escapeTiles[index].GetHashCode();
                        return hash;
                    }
                }
            }
        ...
        internal static bool TryGetHandoffAggressionRank(
            AccessSearchMode mode,
            ...
            out int rank)
        {
            ...
            if (operation != AccessHandoffOperation.Mining
                ...
                return false;
            AccessSearchMode rising = GetRisingMode(travelDirection);
            AccessSearchMode falling = GetRisingMode(new Tile2i(
                -travelDirection.X, -travelDirection.Y));
            int slopeOrder = mode == rising ? 0
                : mode == AccessSearchMode.Flat ? 1
                : mode == falling ? 2
                : -1;
            if (slopeOrder < 0)
                return false;
            rank = operation == AccessHandoffOperation.Mining
                ? slopeOrder
                : 2 - slopeOrder;
            return true;
        }
        ...
        internal static bool TryGetV1DirectLevelingEdgeHandoff(
            AccessSearchSnapshot snapshot,
            Tile2i origin,
            AccessHeightProfile profile,
            Tile2i groundTile,
        ...
        {
            entryDirection = default;
            if (!profile.HasIntegerCorners
                || !snapshot.IsGroundOrCleanupNode(groundTile))
                return false;
            foreach (AccessV1LevelingEdgeSeed seed in
                EnumerateV1LevelingEdgeSeeds(groundTile))
            {
                if (seed.Origin != origin
                    || !TryGetPreciseIntegerLevel(
                        snapshot, groundTile, out int level))
                    continue;
                Tile2i travel = new Tile2i(
                    -seed.OutwardDirection.X,
                    -seed.OutwardDirection.Y);
                int localX = travel.X > 0 ? 0
                    : travel.X < 0 ? 4 : 2;
                int localY = travel.Y > 0 ? 0
                    : travel.Y < 0 ? 4 : 2;
                int target = checked(level * 32);
                if (profile.GetHeight2NumeratorAt(localX, localY) != target)
                    continue;
                profile.GetEdge(seed.OutwardDirection,
                    out int first2, out int second2);
                if (first2 != second2 || first2 != level * 2)
                    continue;
                entryDirection = seed.OutwardDirection;
                return true;
            }
            return false;
        }

        private static bool TryGetPreciseIntegerLevel(
            AccessSearchSnapshot snapshot,
            Tile2i groundTile,
            out int level)
        {
            level = 0;
            if (!snapshot.TryGetPreciseTerrainHeight(
                    groundTile, out float height))
                return false;
            level = (int)Math.Round(height);
            return Math.Abs(height - level) <= 0.0001f;
        }

        internal static bool TryProfileAtEntryLevel(
            AccessSearchMode mode,
            int level,
            Tile2i travelDirection,
            out AccessHeightProfile profile)
        {
            ...
            for (int delta = -2; delta <= 2; delta++)
                if (AccessHeightProfile.TryForMode(
                    ...
                    && profile.GetHeight2NumeratorAt(localX, localY)
                        == target)
                    return true;
            profile = default;
            return false;
            ...
            Tile2i nextGeneratedOrigin)
        {
            if (operation == AccessHandoffOperation.Leveling
                && TryGetV1DirectLevelingEdgeHandoff(
                    snapshot, handoffOrigin, handoffProfile, groundTile,
                    out Tile2i outwardDirection)
                && nextGeneratedOrigin == new Tile2i(
                    handoffOrigin.X - outwardDirection.X,
        ...
        private static float CalculateGeneratedEntryCost(
            ...
            GeneratedPathHistory? history = null)
        {
            ...
            Tile2i? connectedFixedDesignationOrigin =
                snapshot.TryGetFixedProfile(predecessorOrigin, out _)
                    ? predecessorOrigin
                    : (Tile2i?)null;
            Func<Tile2i, AccessSideRayOperation, bool>? rayTermination =
                history != null && history.RayConstraintCount > 0
                    ? history.HasRayAt
                    : null;
            ...
            int clearanceRadius)
        {
            ...
            {
                bool isWork = ray.IsWorkDistance(distance);
                bool isSafetyOnly = ray.IsSafetyOnlyDistance(distance);
                // Results constructed by older callers carry no span masks;
                // retain their historical all-work interpretation.
                if (!isWork && !isSafetyOnly)
                    isWork = true;
            ...
            else
            {
                ...
                else
                {
                    ...
                    result = AccessSideRayCost.Score(
                        ...
                        terminatesAtSameOperationRay:
                            terminatesAtSameOperationRay,
                        projectedTerrainAt: projectedTerrainAt,
                        additionalBlockerAt: additionalBlockerAt);
                }
            }
            if (terminatesAtSameOperationRay == null
                && projectedTerrainAt == null
        ...
        private static bool TryGetRearFaceRayGeometry(
            Tile2i origin,
            AccessHeightProfile profile,
            Tile2i entryDirection,
            out Tile2i firstCorner,
            out float firstHeight,
            out Tile2i secondCorner,
            out float secondHeight,
            out Tile2i rearDirection)
        {
            if (entryDirection.X > 0 && entryDirection.Y == 0)
            {
                firstCorner = origin;
                firstHeight = profile.Nw2 / 2f;
                secondCorner = origin + new RelTile2i(0, 4);
                secondHeight = profile.Sw2 / 2f;
                rearDirection = new Tile2i(-1, 0);
                return true;
            }
            if (entryDirection.X < 0 && entryDirection.Y == 0)
            {
                firstCorner = origin + new RelTile2i(4, 0);
                firstHeight = profile.Ne2 / 2f;
                secondCorner = origin + new RelTile2i(4, 4);
                secondHeight = profile.Se2 / 2f;
                rearDirection = new Tile2i(1, 0);
                return true;
            }
            if (entryDirection.Y > 0 && entryDirection.X == 0)
            {
                firstCorner = origin;
                firstHeight = profile.Nw2 / 2f;
                secondCorner = origin + new RelTile2i(4, 0);
                secondHeight = profile.Ne2 / 2f;
                rearDirection = new Tile2i(0, -1);
                return true;
            }
            if (entryDirection.Y < 0 && entryDirection.X == 0)
            {
                firstCorner = origin + new RelTile2i(0, 4);
                firstHeight = profile.Sw2 / 2f;
                secondCorner = origin + new RelTile2i(4, 4);
                secondHeight = profile.Se2 / 2f;
                rearDirection = new Tile2i(0, 1);
                return true;
            ...
            firstCorner = default;
            firstHeight = 0f;
            secondCorner = default;
            secondHeight = 0f;
            rearDirection = default;
            return false;
        }
        ...
        private static float EstimateDirectWorkCost(
            ...
            AccessHeightProfile profile,
            AccessHandoffOperation operation,
            Func<Tile2i, AccessProjectedTerrainEffect>?
                projectedTerrainAt = null)
        {
            float cost = 0f;
            AddCorner(0, 0);
            ...
            void AddCorner(int localX, int localY)
            {
                ...
                AccessProjectedTerrainEffect projected =
                    projectedTerrainAt?.Invoke(tile) ?? default;
                float cutBase = projected.HasCutWork
                    ? Math.Min(terrainHeight, projected.CutCeiling)
                    : terrainHeight;
                float fillBase = projected.HasFillWork
                    ? Math.Max(terrainHeight, projected.FillFloor)
                    : terrainHeight;
                float delta = plannedHeight - terrainHeight;
                float operationGap = operation == AccessHandoffOperation.Mining
                    ? Math.Max(0f, cutBase - plannedHeight)
                    : operation == AccessHandoffOperation.Dumping
                        ? Math.Max(0f, plannedHeight - fillBase)
                        : delta < 0f
                            ? Math.Max(0f, cutBase - plannedHeight)
                            : Math.Max(0f, plannedHeight - fillBase);
        ...
        private static bool IsV2ProfileBlockedByProjectedTerrain(
            AccessSearchSnapshot snapshot,
            AccessV2History history,
            Tile2i origin,
            AccessHeightProfile profile,
            IReadOnlyCollection<Tile2i>? exemptSnapshotSafetyOrigins,
            IReadOnlyCollection<Tile2i>? exemptHistorySafetyOwners,
            out string reason,
            AccessSearchDiagnostics? diagnostics)
        {
            const float epsilon = 0.0001f;
            for (int y = 0; y <= 4; y++)
            {
                for (int x = 0; x <= 4; x++)
                {
                    ...
                    AccessProjectedTerrainEffect projected =
                        snapshot.GetProjectedDesignationEffectExcept(
                            tile, exemptSnapshotSafetyOrigins);
                    projected.Merge(history.GetProjectedTerrainEffect(
                        tile, exemptHistorySafetyOwners, diagnostics,
                        includeSafety:
                            history.RequiresStrictSelfDisruptionChecks));
                    float height =
                        profile.GetHeight2NumeratorAt(x, y) / 32f;
                    bool hasTerrain = snapshot.TryGetPreciseTerrainHeight(
                        tile, out float terrainHeight);
                    bool isCutting = hasTerrain
                        && height < terrainHeight - epsilon;
                    bool isFilling = hasTerrain
                        && height > terrainHeight + epsilon;
                    if ((projected.HasCutSafety && !isCutting)
                        || (projected.HasFillSafety && !isFilling))
                    {
                        reason = "ProjectedTerrainSafety";
                        return true;
                    }
                    if (projected.HasAmbiguousWork)
                    {
                        reason = "ProjectedTerrainAmbiguous";
                        return true;
                    }
                    if (projected.HasCutWork
                        && height > projected.CutCeiling + epsilon
                        && isFilling)
                    {
                        reason = "ProjectedTerrainReversesCut";
                        return true;
                    }
                    if (projected.HasFillWork
                        && height < projected.FillFloor - epsilon
                        && isCutting)
                    {
                        reason = "ProjectedTerrainReversesFill";
                        return true;
                    }
                }
            }
            ...
            AccessSearchDiagnostics? diagnostics = null)
        {
            if (diagnostics != null)
            {
                diagnostics.V2RayOverlayMaxRawConstraints = Math.Max(
                    diagnostics.V2RayOverlayMaxRawConstraints,
                    history.RayConstraintCount);
                diagnostics.V2RayOverlayMaxCollapsedEntries = Math.Max(
                    diagnostics.V2RayOverlayMaxCollapsedEntries,
                    history.CollapsedRayEntryCount);
            }
            ...
            var snapshotSafetyExemptOrigins = new HashSet<Tile2i>(
                history.GetSnapshotSafetyExemptOrigins());
            if (connectedFixedOrigin.HasValue)
                snapshotSafetyExemptOrigins.Add(
                    connectedFixedOrigin.Value);
            if (current.HasValue)
            {
                for (int lane = 0; lane < 2; lane++)
                {
                    ...
                    if (snapshot.TryGetFixedProfile(
                            predecessorOrigin, out _))
                        snapshotSafetyExemptOrigins.Add(
                            predecessorOrigin);
                }
            }
            foreach (Tile2i contextOrigin
                in transition.LocalContextOrigins)
                if (snapshot.TryGetFixedProfile(contextOrigin, out _))
                    snapshotSafetyExemptOrigins.Add(contextOrigin);

            for (int index = 0; index < transition.Delta.Count; index++)
            {
                AccessV2OriginProfile item = transition.Delta[index];
                if (IsV2ProfileBlockedByProjectedTerrain(
                        snapshot, history, item.Origin, item.Profile,
                        snapshotSafetyExemptOrigins,
                        supersededRayOwners,
                        out string projectedReason, diagnostics))
                    return AccessV2TransitionEvaluation.Reject(
                        projectedReason);
                ...
                directCost += snapshot.LandscapingCostDistanceScale
                    ...
                    * EstimateDirectWorkCost(
                        ...
                        {
                            AccessProjectedTerrainEffect projected =
                                snapshot.GetProjectedDesignationEffect(tile);
                            projected.Merge(
            ...
            if (!TryAddV2ExteriorRays(
                    ...
                    connectedFixedOrigin, operation, history,
                    supersededRayOwners,
                    rayConstraints, ref rayCost,
                    out string rayReason, diagnostics))
                ...
                AccessSearchDiagnostics? diagnostics = null)
        {
            return AccessV2Handoffs.TryCreateDeterministicGroundToVBridge(
                ...
                tile => snapshot.HasV2DumpingPropBlockerAtTile(state, tile),
            ...
            AccessSearchDiagnostics? diagnostics)
        {
            ...
            if (transition.Kind == AccessV2TransitionKind.Strafe
                ...
                return TryAddV2StrafeExteriorRays(
                    ...
                    operation, history, supersededRayOwners,
                    constraints, ref rayCost, out reason, diagnostics);
                ...
                && current.HasValue)
            {
                    ...
                    index++)
                {
                    ...
                    if (!TryAddV2Ray(
                            ...
                            history, supersededRayOwners,
                            out reason, diagnostics))
                        return false;
                }
                reason = string.Empty;
                return true;
            ...
            AccessSearchDiagnostics? diagnostics)
        {
            ...
            AccessV2OriginProfile predecessorOuter = default;
            bool hasPredecessorOuter = false;
            for (int index = 0; index < transition.Delta.Count; index++)
            {
                AccessV2OriginProfile candidate = transition.Delta[index];
                Tile2i successorOrigin = AccessV2Geometry.Add(
                    candidate.Origin, next.EntryDirection);
                if (!transition.Delta.Any(
                        item => item.Origin == successorOrigin))
                    continue;
                predecessorOuter = candidate;
                hasPredecessorOuter = true;
                break;
            }
            if (!hasPredecessorOuter
                || !TryGetRearFaceRayGeometry(
                    predecessorOuter.Origin,
                    predecessorOuter.Profile,
                    next.EntryDirection,
                    out Tile2i rearFirstCorner,
                    out float rearFirstHeight,
                    out Tile2i rearSecondCorner,
                    out float rearSecondHeight,
                    out Tile2i rearDirection))
            {
                reason = "StrafeRearRayInvalidGeometry";
                return false;
            }
            if (!TryAddV2Ray(
                    snapshot, constraints, ref rayCost,
                    predecessorOuter.Origin,
                    rearFirstCorner, rearFirstHeight,
                    rearDirection, connectedFixedOrigin, operation,
                    history, supersededRayOwners, out reason,
                    diagnostics)
                || !TryAddV2Ray(
                    snapshot, constraints, ref rayCost,
                    predecessorOuter.Origin,
                    rearSecondCorner, rearSecondHeight,
                    rearDirection, connectedFixedOrigin, operation,
                    history, supersededRayOwners, out reason,
                    diagnostics))
                return false;
            ...
            AccessSearchDiagnostics? diagnostics)
        {
            AccessSideRayResult ray = ScoreExitCorner(
                ...
                projectedTerrainAt:
                    history.RayConstraintCount == 0
                        ? null
                        : (tile, rayOperation) =>
                            history.GetProjectedTerrainEffect(
                                tile, supersededRayOwners,
                                diagnostics,
                                includeSafety:
                                    history.RequiresStrictSelfDisruptionChecks),
                additionalBlockerAt:
                    history.RequiresStrictSelfDisruptionChecks
                        ? tile => history.HasGeneratedProfileAt(
                                tile, supersededRayOwners)
                            ? "SideRayPathOrigin"
                            : null
                        : null);
            ...
            for (int index = 0; index < added.Count; index++)
            {
                ...
                constraints.Add(new AccessRayHeightConstraint(
                    ...
                    constraint.Height,
                    ownerOrigin,
                    constraint.IsSafetyOnly));
            }
            reason = string.Empty;
            return true;
        }

        private static float EstimateMarginalProjectedRayWorkCost(
            AccessSearchSnapshot snapshot,
            AccessV2History history,
            IEnumerable<AccessRayHeightConstraint> existingConstraints,
            IReadOnlyList<AccessRayHeightConstraint> addedConstraints,
            AccessSearchDiagnostics? diagnostics)
        {
            var beforeByTile =
                new Dictionary<Tile2i, AccessProjectedTerrainEffect>();
            foreach (AccessRayHeightConstraint existing
                in existingConstraints)
                Merge(existing, beforeByTile);

            var addedByTile =
                new Dictionary<Tile2i, AccessProjectedTerrainEffect>();
            for (int index = 0; index < addedConstraints.Count; index++)
                Merge(addedConstraints[index], addedByTile);
            ...
            {
                if (!snapshot.TryGetPreciseTerrainHeight(
                        pair.Key, out float terrainHeight))
                    continue;

                AccessProjectedTerrainEffect before =
                    snapshot.GetProjectedDesignationEffect(pair.Key);
                before.Merge(history.GetProjectedTerrainEffect(
                    pair.Key, null, diagnostics, includeSafety: false));
                if (beforeByTile.TryGetValue(
                        pair.Key,
                        out AccessProjectedTerrainEffect pending))
                    before.Merge(pending);

                if (pair.Value.HasCutWork)
                {
                    float oldCeiling = before.HasCutWork
                        ? Math.Min(terrainHeight, before.CutCeiling)
                        : terrainHeight;
                    float newCeiling = Math.Min(
                        oldCeiling, pair.Value.CutCeiling);
                    cost += Math.Max(0f, oldCeiling - newCeiling);
                }
                if (pair.Value.HasFillWork)
                {
                    float oldFloor = before.HasFillWork
                        ? Math.Max(terrainHeight, before.FillFloor)
                        : terrainHeight;
                    float newFloor = Math.Max(
                        oldFloor, pair.Value.FillFloor);
                    cost += Math.Max(0f, newFloor - oldFloor);
                }
            }
            return cost;
            ...
            static void Merge(
                ...
                IDictionary<Tile2i, AccessProjectedTerrainEffect> effects)
            {
                if (constraint.IsSafetyOnly)
                    return;
                effects.TryGetValue(
                    constraint.Tile, out AccessProjectedTerrainEffect effect);
                ...
                effects[constraint.Tile] = effect;
            }
        }
            ...
            AccessSearchDiagnostics? diagnostics = null)
        {
            ...
            GeneratedPathHistory replayHistory = GeneratedPathHistory.Empty;
            for (int pathIndex = 0; pathIndex < path.Count; pathIndex++)
            {
                ...
                else if (node.Mode != AccessSearchMode.Existing)
                {
                    if (TryGetProfile(snapshot, node, out AccessHeightProfile profile))
                    {
                        ...
                        AccessSideRayResult turnOuterRay =
                            new AccessSideRayResult(0f, 0f, 0, false, false);
                        Tile2i turnCorner = default;
                        Tile2i turnDirection = default;
                        IReadOnlyList<AccessRayHeightConstraint>
                            turnRayConstraints =
                                Array.Empty<AccessRayHeightConstraint>();
                        ...
                        {
                            turnOuterRay = ScoreTurnOuterCorner(
                            ...
                            turnRayConstraints = BuildRayHeightConstraints(
                                snapshot, turnCorner,
                                GetProfileCornerHeight(
                                ...
                                turnDirection, turnOuterRay,
                                GetGeneratedWorkOperation(
                                    predecessor.HandoffOperation),
                                snapshot.VehicleClearanceRadius);
                        }
                        IReadOnlyList<AccessRayHeightConstraint>
                            replayRayConstraints = MergeRayHeightConstraints(
                                landscapingCost.RayHeightConstraints,
                                turnRayConstraints);
                        replayHistory = replayHistory.WithGenerated(
                            ...
                            MergeDisturbedRayTiles(
                                landscapingCost.DisturbedRayTiles,
                                turnCorner, turnDirection,
                                turnOuterRay.DisturbedDistance,
                                snapshot.VehicleClearanceRadius),
                            rayHeightConstraints: replayRayConstraints);
                    ...
                    && TryGetProfile(snapshot, node, out AccessHeightProfile profile))
                {
                            ...
                            out AccessHeightProfile predecessorProfile))
                    {
                        ...
                        turnRay = ScoreTurnOuterCorner(
                            ...
                            (tile, rayOperation) =>
                                replayHistory.HasRayAt(tile, rayOperation),
                            out turnCorner, out turnDirection);
                    ...
                    replayHistory = replayHistory.WithGenerated(
                        node.Position, profile,
                        MergeDisturbedRayTiles(
                            landscaping.DisturbedRayTiles,
                            turnCorner, turnDirection,
                            turnRay.DisturbedDistance,
                            snapshot.VehicleClearanceRadius),
                        rayHeightConstraints: MergeRayHeightConstraints(
                            landscaping.RayHeightConstraints,
                            turnRayConstraints));
                }
                predecessor = node;
            }
        ...
        internal sealed class GeneratedPathHistory
        {
                ...
                IReadOnlyList<AccessRayHeightConstraint>? rayHeightConstraints = null)
            {
                ...
                RayConstraintCount = parent.RayConstraintCount
                    + m_cutSupportCeilings.Count
                    + m_fillSurfaceFloors.Count;
                m_hasGenerated = true;
            ...
            public bool IsProfileBlockedByRayEnvelope(
                ...
                => IsProfileBlockedByRayEnvelope(
                    null, origin, profile, out blockingOperation);
                    ...
                    history = history.m_parent)
                {
                    for (int y = 0; y <= 4; y++)
                    {
                        for (int x = 0; x <= 4; x++)
                        {
                            ...
                            AccessTerrainSampleKind sampleKind = snapshot != null
                                ? snapshot.GetSideRayTerrainSample(
                                    tile, out terrainHeight)
                                : AccessTerrainSampleKind.MissingSnapshot;
                            bool hasTerrain = sampleKind
                                    != AccessTerrainSampleKind.MissingSnapshot
                                && sampleKind
                            ...
                            {
                                if (hasTerrain && height < terrainHeight - epsilon)
                                    continue;
                ...
                AccessSideRayOperation operation)
            {
                    ...
                    history = history.m_parent)
                {
                    if (operation == AccessSideRayOperation.Cut
                        && history.m_cutSupportCeilings.ContainsKey(tile))
                        return true;
                    if (operation == AccessSideRayOperation.Fill
                        && history.m_fillSurfaceFloors.ContainsKey(tile))
                        return true;
                }
                return false;
            }
